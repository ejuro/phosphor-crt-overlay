...
**The power transitions cost only while they run.** Switching on feels like powering a terminal: the desktop cuts to black in about 30ms, holds properly dark while the tube warms, then the picture rises out of the black — slow at first, the way a phosphor comes up to brightness, with scanlines, glow and glass already lit as it appears. Deliberately not the television line-and-dot. How long that takes is the panel's *Warm-up* slider, from 0.4s to 3s, dark for the first 40% of it and rising through the rest; the default is 1.4s. The cut to black stays fast whatever it is set to, because the sudden darkness is the character of the thing.
