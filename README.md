...
- From anywhere: `omarchy-shell phosphor toggle`, or `enable` / `disable` / `preset <id>` / `cabinet on|off` / `warmup on|off` / `fade <seconds>` / `save` / `keep` / `reset` / `status`. Individual controls too: `set scanline 0.3`, `set trueWarp true`, `stock scanline`.
...
**The power transitions cost only while they run.** Switching on feels like powering a terminal: the desktop cuts to black in about 30ms, holds properly dark while the tube warms, then the picture rises out of the black — and not merely by getting brighter. The beam starts loose, so the image is defocused as well as dim and resolves out of a soft glow as it comes up, with scanlines, glow and glass already lit as it appears. Deliberately not the television line-and-dot. How long that takes is the panel's *Warm-up* slider, from 0.4s to 3s, dark for the first 40% of it and rising through the rest; the default is 1.4s. The cut to black stays fast whatever it is set to, because the sudden darkness is the character of the thing.

Switching off is the asymmetric opposite and is not scaled by that slider: the phosphor dies over about 110ms, the screen holds dark for a beat, and the shader is cleared while it is still black so the plain desktop returns in one step, with no second fade. Around 270ms, always.

Transitions need `debug:damage_tracking = 0`, because Hyprland only feeds a shader `time` in that mode, and Phosphor restores your setting the moment the transition ends. The clock starts when Hyprland *loads* the shader, not when you press the key, so the swap back is timed from the apply: writing the file and the `hyprctl` round trip cost 100-200ms, and timing it from the keypress cut the fade off while the screen was still black, which read as a blink. Coming back, the shader is cleared at damage mode 1 for one whole-monitor redraw before your setting goes back, so nothing the tube drew is left smeared in a region nothing happens to touch. The panel's *Power fade* switch turns both off if you would rather changes be instant.

**Damage mode 0 does not mean "render continuously".** It means damage is not tracked, so when Hyprland *does* render it redraws everything — but it still only renders when something damages the monitor, and a shader's `time` advancing is not damage. On an idle desktop a warm-up therefore repaints only on the two or three occasions something else happens to change, and a smooth fade arrives as two or three visible steps. Screenshots hide this completely: a screencopy request forces a frame, so any measurement taken with `grim` shows a perfectly smooth ramp nobody ever sees. It shows up in GPU utilisation, which sat at idle through a three-second fade apart from one brief burst.

So while a timed shader is loaded, Phosphor maps a 1×1 transparent click-through layer surface that commits a buffer every frame. That commit is the damage, and it keeps the monitor redrawing for as long as the transition runs — the same three-second fade now holds 12-32% GPU throughout. It is also what makes *Flicker* work at all rather than sitting on whatever frame was last drawn. The surface exists only while a transition runs or the tube is live; with flicker off, a settled tube goes back to idle and the surface is gone.

**Flicker is the priced exception.** Every other slider — brightness, contrast, colour, scanlines, scan pitch, mask, bloom, vignette, convergence, grain, glass curve — changes a shader that still costs nothing at idle. Flicker above zero needs a clock, and Hyprland only feeds a shader `time` with damage tracking fully off; a flickering tube also has to keep damaging the screen to make that clock reach it, so it really does redraw every frame for as long as flicker is set — measured at 24-29% GPU against 6% idle. Grain comes alive along with it. The panel says so next to the slider; at zero the tube goes back to the free static shader.
